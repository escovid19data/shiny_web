falleUI <- function(id) {
  ...
  div(style = "margin-left:30px; margin-right:30px;",
    ...
    fluidRow(
      column(12,
             column(3,
                    selectInput(ns("falle_select1"), "Variable",
                                choices = c("Total fallecidos", "Fallecidos diarios", "Fallecidos por 100.000 h",
                                            "Tasa de mortalidad %", "Media móvil semanal"))
             ),
             column(3,
                    selectInput(ns("falle_select_province"),"Provincias", choices= c("Todas" = "",unique(cases_data$province)),
                                multiple = TRUE, 
                    )
                    
             ),
             column(3,
                    sliderTextInput(ns("falle_select_fecha"), "Periodo",
             ...
             column(3,style = "padding-top: 20px; text-align: center;",
                    awesomeCheckbox(ns("falle_logar100"),
    ...
    fluidRow(
      column(12,
             plotlyOutput(ns("falle_graph"),width = "100%")
      )
    ),
    fluidRow(
      column(12,
             plotlyOutput(ns("falle_graph2"),width = "98%")
      )
    )
  )
}

falleServer <- function(id) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$title <- renderUI({
        div(tags$h1(paste(input$falle_select1, "del", input$select_fecha[1], "al",input$select_fecha[2], sep = " "), 
                    style = "text-align:center; padding-botom: 25px;"))
      })
      
      logar <- reactive({
        if(input$falle_logar100 == TRUE){"log"}
